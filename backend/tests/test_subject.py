...
def test_create_subject_invalid_faculty(
...
def test_create_subject_invalid_department(
...
def test_create_subject_unauthorized(
    ...
    faculty = Faculty(
        name="Dr. Unauthorized Professor", department_id=test_department.id
    )
    session.add(faculty)
    session.commit()
    session.refresh(faculty)
...
def test_get_subject_by_id(
    ...
    subject = Subject(
        name="ID Test Subject", faculty_id=faculty.id, department_id=test_department.id
    )
    session.add(subject)
    session.commit()
...
def test_update_subject(
    ...
    subject = Subject(
        name="Original Subject", faculty_id=faculty.id, department_id=test_department.id
    )
    session.add(subject)
    session.commit()
    session.refresh(subject)
    ...
    response = client.put(
        f"/subject/{subject.id}", json=update_data, headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_update_subject_partial(
    client: TestClient,
    auth_headers: dict,
    session: Session,
    test_department: Department,
...
def test_update_subject_invalid_faculty(
    client: TestClient,
    auth_headers: dict,
    session: Session,
    test_department: Department,
    ...
    faculty = Faculty(
        name="Dr. Invalid Test Professor", department_id=test_department.id
    )
    session.add(faculty)
    session.commit()
    session.refresh(faculty)
...
def test_update_subject_invalid_department(
    client: TestClient,
    auth_headers: dict,
    session: Session,
    test_department: Department,
    ...
    faculty = Faculty(
        name="Dr. Invalid Dept Professor", department_id=test_department.id
    )
    session.add(faculty)
    session.commit()
    session.refresh(faculty)
...
def test_update_subject_not_found(
    ...
    faculty = Faculty(
        name="Dr. Non-existent Professor", department_id=test_department.id
    )
    session.add(faculty)
    session.commit()
    session.refresh(faculty)
...
def test_subject_unauthorized_access(
    client: TestClient, session: Session, test_department: Department
):
