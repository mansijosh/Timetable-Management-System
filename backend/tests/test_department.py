...
def test_create_department(client: TestClient, auth_headers: dict):
    ...
    department_data = {"name": "Mathematics", "year": 2024}
...
def test_create_department_unauthorized(client: TestClient):
    ...
    department_data = {"name": "Physics", "year": 2024}
...
def test_update_department(
    ...
    update_data = {"name": "Advanced Computer Science", "year": 2025}
    response = client.put(
        f"/department/{test_department.id}", json=update_data, headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_update_department_partial(
    client: TestClient, auth_headers: dict, test_department: Department
):
    """Test partial update of department"""
    update_data = {"name": "Updated Computer Science"}
    response = client.put(
        f"/department/{test_department.id}", json=update_data, headers=auth_headers
    )
    assert response.status_code == 200
...
def test_update_department_not_found(client: TestClient, auth_headers: dict):
    ...
    update_data = {"name": "Physics", "year": 2024}
...
def test_department_unauthorized_access(
    client: TestClient, test_department: Department
):
