...
def test_create_faculty(client: TestClient, auth_headers: dict, test_department: Department):
...
def test_get_faculty_by_id(client: TestClient, auth_headers: dict, session: Session, test_department: Department):
...
def test_update_faculty(client: TestClient, auth_headers: dict, session: Session, test_department: Department):
    ...
    response = client.put(f"/faculty/{faculty.id}", json=update_data, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["name"] == "Dr. Updated Name"
    assert data["id"] == faculty.id

def test_update_faculty_partial(client: TestClient, auth_headers: dict, session: Session, test_department: Department):
    ...
    update_data = {
        "name": "Dr. Partially Updated"
        # department_id not provided, should remain unchanged
    }
    response = client.put(f"/faculty/{faculty.id}", json=update_data, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
def test_update_faculty_not_found(client: TestClient, auth_headers: dict, test_department: Department):
    """Test updating non-existent faculty"""
    update_data = {
        "name": "Dr. Non-existent",
        "department_id": test_department.id
    }
...
def test_delete_faculty(client: TestClient, auth_headers: dict, session: Session, test_department: Department):
...
def test_faculty_unauthorized_access(client: TestClient, session: Session, test_department: Department):
