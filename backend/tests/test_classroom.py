...
def test_get_classrooms(
    client: TestClient, auth_headers: dict, test_classroom: Classroom
):
...
def test_update_classroom(
    ...
    response = client.put(
        f"/classroom/{test_classroom.id}", json=update_data, headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
...
def test_update_classroom_invalid_department(
    client: TestClient, auth_headers: dict, test_classroom: Classroom
):
...
def test_update_classroom_not_found(
    client: TestClient, auth_headers: dict, test_department: Department
):
